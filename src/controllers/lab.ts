...
async function parseLabWithGemini(base64Image: string, mimeType: string) {
...
export async function uploadLab(req: Request, res: Response) {
  try {
    const file = req.file;
    const { 
      lab_date, family_history, accident_history, serious_illness_history,
      is_smoker, smoking_duration, drinks_alcohol, alcohol_duration,
      ktp_number
    } = req.body;

    if (!file || !req.user) return res.status(400).json({ error: 'File and Authenticated user required' });

    const ext = file.originalname.split('.').pop()?.toLowerCase() || 'jpg';
    const filename = `lab_${req.user.id}_${Date.now()}.${ext}`;
    const uploadDir = path.join(process.cwd(), '../mobile-pwa/public/uploads/labs');
    ...
    try {
      ...
      await execute(
        'UPDATE lab_results SET status = ?, recommendation = ?, ai_raw_response = ? WHERE id = ?', 
        [status, recommendation, JSON.stringify(extractedValues), labResultId]
      );
    } catch (e) {
      status = 'needs_program';
      await execute('UPDATE lab_results SET status = ? WHERE id = ?', [status, labResultId]);
    }
...
export async function getLatestLab(req: Request, res: Response) {
  if (!req.user) return res.status(401).send();
  const labResult = await queryOne('SELECT * FROM lab_results WHERE user_id = ? ORDER BY created_at DESC LIMIT 1', [req.user.id]);
  if (!labResult) return res.json({ labResult: null, parameters: [] });
  const parameters = await query('SELECT * FROM lab_parameters WHERE lab_result_id = ?', [labResult.id]);
  res.json({ labResult, parameters });
}

export async function getLabById(req: Request, res: Response) {
  if (!req.user) return res.status(401).send();
  const labResult = await queryOne('SELECT * FROM lab_results WHERE id = ? AND user_id = ?', [req.params.id, req.user.id]);
  if (!labResult) return res.status(404).json({ error: 'Not found' });
  const parameters = await query('SELECT * FROM lab_parameters WHERE lab_result_id = ?', [labResult.id]);
  res.json({ labResult, parameters });
}

export async function extractLab(req: Request, res: Response) {
  try {
    const file = req.file;
    if (!file || !req.user) return res.status(400).json({ error: 'File and Authenticated user required' });
...
export async function saveManualLab(req: Request, res: Response) {
  try {
    if (!req.user) return res.status(401).send();
    const { 
      lab_date, parameters,
    ...
    let hasAbnormal = false;
    for (const [key, valueStr] of Object.entries(parameters || {}) as any) {
    ...
    await execute(
      'UPDATE lab_results SET status = ?, recommendation = ? WHERE id = ?',
      [status, recommendation, labResultId]
    );

    // If there is a requested program, update it to use this new lab result
    await execute(
      'UPDATE wellness_programs SET lab_result_id = ? WHERE user_id = ? AND status = \'requested\'',
      [labResultId, req.user.id]
    );
